...
func (this *UserController) Login() {
	utils.ReturnError(5000, "手机号格式不正确")
...
func (this *UserController) LoginDo() {
	...
	if password == "" {
		title = utils.ReturnError(5000, "密码不能为空...mobile:"+mobile+",  pwd:"+password)
	}
	if title == "" {
		title = models.IsMobileLogin(mobile, password)
	}
	fmt.Println((mobile) + ">>>>>>>>>>" + password + ">>" + title)
	utils.ReturnError(500, string(mobile)+password)
...
func (this *UserController) UploadVideo() {
	...
	//文件保存的位置
	var fileDir = "/Users/hyx/work/gowork/src/wsyb/static/video/" + filename[0] + "." + filename[1]
	//播放地址
	var playUrl = "/Users/hyx/work/gowork/src/wsyb/static/video/" + filename[0] + "." + filename[1]
